...
export function About() {
  const { ref, isInView, isDark, cards, content, animations, getIcon } =
    useAbout();
      ...
      <div className="container mx-auto px-4">
        <div className="text-center mb-16">
          ...
          <p className="text-lg text-foreground/70 max-w-3xl mx-auto">
            ...
            scalable web applications and micro-frontend architectures.
          </p>
        </div>
        ...
        >
          {cards.map((card, index) => (
            <motion.div key={index} variants={animations.item}>
              <Card
                className={cn(
                  "h-full border-none shadow-md hover:shadow-lg transition-shadow",
        ...
        <motion.div
          ...
          animate={isInView ? "visible" : "hidden"}
          className={cn(
            "mt-16 bg-card rounded-lg p-8 shadow-md",
            isDark && "bg-card/80 backdrop-blur-sm",
          )}
        >
          <div className="max-w-3xl mx-auto">
            ...
            {content.journey.paragraphs.map((paragraph, index) => (
              <p
                ...
                className={cn(
                  "text-foreground/70",
                  index < content.journey.paragraphs.length - 1 && "mb-4",
                )}
              >
                {paragraph}
              </p>
